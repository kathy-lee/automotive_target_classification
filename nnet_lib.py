...
def cnn_a(train_data, train_label):
    ...
    model.add(
        Conv2D(32, [5, 5], activation='relu', kernel_initializer='he_uniform', kernel_regularizer='l2', name='conv_2'))
    model.add(MaxPooling2D())
    model.add(Flatten())
...
def rnn_a(train_data, train_label):
    train_data, val_data, train_label, val_label = train_test_split(train_data, train_label, test_size=0.1,
                                                                    random_state=42)
    print("Split training data into training and validation data:\n")
    print("training data: %d" % train_data.shape[0])
