...
class CyclicLR(Callback):
    ...
    def on_train_begin(self, logs={}):
        ...
        if self.clr_iterations == 0:
            set_value(self.model.optimizer.lr, self.base_lr)
        else:
            set_value(self.model.optimizer.lr, self.clr())
    ...
    def on_batch_end(self, batch, logs=None):
        ...
        if self.cycle_momentum:
            if self.overhump:
                ...
                new_momentum = self.max_momentum - current_percentage * (
                        self.max_momentum - self.min_momentum)
            set_value(self.model.optimizer.beta_1, new_momentum)
            self.history.setdefault('momentum', []).append(get_value(self.model.optimizer.beta_1))
        ...
        if self.patience:
            ...
            if self.monitor_op(current, self.best):
                ...
                if self.wait >= self.patience:
                    min_lr = 1e-7
                    current_lr = float(get_value(self.model.optimizer.lr))
